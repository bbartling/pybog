...
def select_best_tools(
    ...
    log_steps.append({"timestamp": datetime.now().isoformat(), "step": "Analyze Tools", "details": {"goal": goal}})
    ...
    if described_tools_count < 5:
        ...
        print("   The LLM's initial choice may be less accurate. Proceeding with analysis...")
        log_steps.append({"timestamp": datetime.now().isoformat(), "step": "Quality Gate", "details": {"level": "warning", "message": warning_msg}})


    if not api_docs:
        error_msg = "API documentation is missing. Cannot continue."
        print(f"❌ Agent: {error_msg}")
        log_steps.append({"timestamp": datetime.now().isoformat(), "step": "Error", "details": {"message": error_msg}})
        return []
    ...
    simplified_tools = [{"name": tool["name"], "description": tool.get("description", "No description provided.")} for tool in tools]
...
"""
    ...
    log_steps.append({"timestamp": datetime.now().isoformat(), "step": "LLM Consultation", "details": {"prompt_sent": prompt}})


    try:
        if source_code_docs:
            print("   -> Sending enhanced prompt to Gemini with API docs, examples, AND source code...")
        else:
            print("   -> Sending enhanced prompt to Gemini with API docs and examples...")
        ...
        try:
            ...
            print(f"❌ Agent: Gemini returned a response that could not be parsed as JSON: {response.text}")
            return []

        
        log_steps.append({"timestamp": datetime.now().isoformat(), "step": "LLM Response", "details": {"selected_tool_names": best_tool_names}})
        ...
        log_steps.append({"timestamp": datetime.now().isoformat(), "step": "Error", "details": {"source": "Gemini API", "message": str(e)}})
        return []

def execute_tool(tool_name: str, output_dir: str, log_steps: list) -> Optional[Dict[str, Any]]:
    ...
    log_steps.append({"timestamp": datetime.now().isoformat(), "step": "Execute Tool", "details": {"tool_name": tool_name, "endpoint": endpoint, "payload": payload}})
    ...
    try:
        ...
        print(f"❌ Agent: {error_msg}")
        log_steps.append({"timestamp": datetime.now().isoformat(), "step": "Error", "details": {"source": "Tool Execution", "message": error_msg}})
        if e.response:
...
if __name__ == "__main__":
    ...
    try:
        ...
        if not available_tools or not api_docs:
            ...
            chosen_tools = select_best_tools(available_tools, user_goal, api_docs, log_steps=log_data["steps"])
            ...
            if not chosen_tools:
                ...
                log_data["steps"].append({"timestamp": datetime.now().isoformat(), "step": "Fetch Source Code Docs", "details": {"success": source_docs is not None}})
                
                if source_docs:
                    # This second call includes the powerful source_code_docs context.
                    chosen_tools = select_best_tools(available_tools, user_goal, api_docs, log_steps=log_data["steps"], source_code_docs=source_docs)
                else:
                    print("❌ Agent: Could not fetch source code context for retry. Halting.")
            ...
            if chosen_tools:
                ...
                for i, tool in enumerate(chosen_tools):
                    ...
                    execution_result = execute_tool(tool['name'], output_dir=output_destination, log_steps=log_data["steps"])
                    if execution_result:
                        print(f"✅ Agent: Successfully executed '{tool['name']}'. Halting further attempts.")
                ...
                if execution_result:
                    ...
                    log_data["final_result"] = {"status": "failed", "reason": "All selected tools failed."}

            else:
                print("\n❌ Agent: Could not select any suitable tools, even after a retry. Please review the user goal or the available tools.")
                log_data["final_result"] = {"status": "failed", "reason": "Could not select any suitable tools."}

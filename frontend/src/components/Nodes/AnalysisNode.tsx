...
import { NodeChrome, StatusPuck } from '../flow/Chrome';
...
const AnalysisNode: React.FC<NodeProps<AnalysisData>> = ({ data }) => {
  ...
  return (
    <div className={(data as any)?.approving || (data as any)?.processing ? 'rf-node-pulse' : ''}>
      <NodeChrome title="Analysis" tone="tool">
        <div style={{ whiteSpace: 'pre-wrap', marginBottom: 8 }}>{data?.content || 'Analysis results available.'}</div>
        {summary}

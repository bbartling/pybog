...
class BogFolderBuilder:
    ...
    def add_boolean_writable(self, name, default_value=False):
        """
        Helper to register a standard control:BooleanWritable component
        using fallback->value (Niagara format).
        """
        standard_actions = {
            "emergencyActive": "h",
            "emergencyInactive": "h",
    ...
    def add_numeric_writable(self, name, default_value=0.0):
        ...
        """
        standard_actions = {"emergencyOverride": "h", "emergencyAuto": "h"}
        self.add_component(
    ...
    def add_link(self, source_comp_name, source_slot, target_comp_name, target_slot):
        ...
        link_type = "b:Link"
        # Auto-conversion: BooleanWritable → kitControl:Equal (numeric input)
        if source_type == "control:BooleanWritable" and "kitControl:" in target_type:
    ...
    def _build_xml(self):
        ...
        for level in levels[1:]:
            ...
            for name in sorted_level:
                sources = [
                    l["source_name"] for l in self._links if l["target_name"] == name
                ]
                if sources:
                    parent_y = avg_input_y(name)
                    expanded_positions.append((name, parent_y))
                else:
                    expanded_positions.append((name, self.START_Y))

            # Spread children evenly if multiple targets from same parent
            total_height = len(expanded_positions) * self.Y_INCREMENT
            start_y = (
                self.START_Y
                + (len(levels[0]) * self.Y_INCREMENT // 2)
                - (total_height // 2)
            )

            # Center each block with spacing
            for i, (name, _) in enumerate(expanded_positions):

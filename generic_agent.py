...
def main(argv: Optional[List[str]] = None) -> None:
    ...
    while attempts < args.max_iters:
        ...
        if attempts == 1:
            code = llm_generate_script(
                ...
                examples_blob,
                bog_file_name,
                label=f"generate (attempt {attempts})",
            )

        else:
            code = llm_fix_script(
                model, description, last_code, last_err, examples_blob, bog_file_name
            )
    ...
    if args.output:
        ...
        try:
            ...
            print(
                f"⚠️ Could not move to requested output path ({e}). Keeping: {created_bog}"
            )
